window.vxgplayer = function(id, options_){
	window.vxgplayer.version="1.8.40"; //version is updated by 'npm run build'
...
window.vxgplayer.initOptions = function(self, options){
	...
	self.m.audio_parameter = '0'; // (self.player.hasAttribute('audio') && parseInt(self.player.getAttribute('audio', 10)) == 0) ? '0' : '1';
	self.m.mute_parameter = '0'; // (self.player.hasAttribute('mute') && parseInt(self.player.getAttribute('mute', 10)) == 1) ? '1' : '0';
}
...
window.vxgplayer.initVolumeControls = function(self, onloadsettings){
	self.m.volume = self.options.volume || 0;
